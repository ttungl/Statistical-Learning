...
```{r}
require(ISLR) # use "install.packages('ISLR')"
require(tree) # use "install.packages('tree')"
attach(Carseats)
hist(Sales)
High=ifelse(Sales <= 8,"No","Yes") # if sales is not high=> No, otherwise, yes.
...
```{r}
tree.carseats=tree(High~.-Sales,data=Carseats) # everything except Sales.
summary(tree.carseats)
plot(tree.carseats)
text(tree.carseats,pretty=0) # adjust font "cex=.6""
```
For a detailed summary of the tree, print it:
```{r}
tree.carseats # extract details from the tree.
```
...
```{r}
rf.boston=randomForest(medv~.,data=Boston,subset=train) # all after medv.
rf.boston
```
...
abline(h=min(test.err),col="red") # draw a line
```
