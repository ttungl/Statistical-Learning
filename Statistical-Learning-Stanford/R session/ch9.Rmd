...
library(e1071) # use "install.packages('e1071')" before this.
...
make.grid=function(x,n=75){
  grange=apply(x,2,range) # get the range of each var in x.
  x1=seq(from=grange[1,1],to=grange[2,1],length=n) # go from lower vals to upper vals
  x2=seq(from=grange[1,2],to=grange[2,2],length=n)
  expand.grid(X1=x1,X2=x2) # make a lattice
  }
xgrid=make.grid(x)
...
6 data points within margin of +/- 1.
```
...
```{r}
plot(x,col=y+1)
dat=data.frame(y=factor(y),x) # two dims
fit=svm(factor(y)~.,data=dat,scale=FALSE,kernel="radial",cost=5) # fit classifier.
```
...
```{r}
xgrid=expand.grid(X1=px1,X2=px2) # use expand to create grid of values
ygrid=predict(fit,xgrid)
plot(xgrid,col=as.numeric(ygrid),pch=20,cex=.2)
points(x,col=y+1,pch=19) # add data points
```
