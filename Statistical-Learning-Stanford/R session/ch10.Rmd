...
```{r}
set.seed(101)
x=matrix(rnorm(100*2),100,2) # r:100; c:2
xmean=matrix(rnorm(8,sd=4),4,2) # r:4; c:2
which=sample(1:4,100,replace=TRUE) # sample 1->4
x=x+xmean[which,] # pick hundred rows matrix with 2 cols from x.
...
points(x,col=c(4,3,2,1)[which],pch=19) # order
```
...
```{r}
hc.cut=cutree(hc.complete,4) # cut level 4
table(hc.cut,which)
